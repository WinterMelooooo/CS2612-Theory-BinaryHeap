...
Definition inner_body: Z -> program unit (Z + Z) :=
  fun j =>
    assert (0 <= j < Zlength str);;
    assert (j < Zlength vnext);;
    choice (test(ch = Znth j str default);;
            return (inr (j + 1)))
            (test(ch <> Znth j str default);;
            choice (test(j = 0);;
                    return (inr (0))) 
                    (test(j <> 0);;
                    return (inl (Znth (j-1) vnext 0)))).

Definition inner_loop: Z -> program unit Z :=
  whilebreak inner_body.
...
Lemma inner_jrange_inv (i j: Z):
  ...
  jrange j vnext ->
  Hoare (fun _ => True)
        (x <- inner_body default patn vnext (Znth i text default) j;; inl_case x)
        (fun j' _ => jrange j' vnext).
Proof.
  intros; unfold jrange in *.
  unfold inner_body. 
  prog_nf; hoare_step; try lia.
...
Lemma inner_jrange_inr1 (j: Z):
  jrange j vnext -> 0 <= j+1 <= Zlength vnext.
Proof.
  intros; hnf in H; lia.
Qed.

Lemma inner_jrange_inr2:
  0 <= 0 <= Zlength vnext.
Proof.  
...
Proof.
  intros H Hf H0 Hj.
  unfold jrange in *.
  unfold inner_body; prog_nf; hoare_step; try lia.
  clear σ H5; unfold is_prefix_func in Hf.
  specialize (Hf (j-1) ltac:(lia)). 
  replace (j - 1 + 1) with j in Hf by lia.
...
Proof.
  ...
  split. apply inner_jrange_inr1; tauto.
...
Lemma inner_body_prop_inr (j: Z):
  Zlength vnext <= Zlength patn ->
  0 <= i < Zlength text ->
  inner_inv j ->
  Hoare (fun _ => True)
        (x <- inner_body default patn vnext (Znth i text default) j;; inr_case x)
...
Proof.
  ...
  - unfold inner_pre, inner_inv, jrange in *; lia.
  - apply inner_body_prop_inr1; auto.
  - eapply inner_body_prop_inr2; eauto.
Qed.
...
Proof.
  ...
  - intros; apply Hoare_sum; hoare_intros.
    eapply inner_body_prop_inv; eauto.
    apply inner_body_prop_inr; tauto.
Qed.
...
Proof.
  ...
  unfold constr_loop.
  eapply Hoare_bind.
  apply range_iter_ind with (P := fun k (res: list Z * Z) _ =>
                              let (vnext, j) := res in                      
...
Lemma match_body_inl_prop:
  Hoare (fun _ => True)
        (x <- match_body default patn text vnext i j;; inl_case x)
...
Lemma match_body_inr_prop:
  Hoare (fun _ => True)
        (x <- match_body default patn text vnext i j;; inr_case x)
...
Lemma match_body_prop:
  Hoare (fun _ => True)
        ...
        (fun res _ => 
            match res with
            | inr z => first_occur patn text z
            | inl z => match_inv (i+1) z 
            end).
Proof.
  apply Hoare_sum.
  - apply match_body_inl_prop; auto.
  - apply match_body_inr_prop; auto. 
Qed.
